classdef math_model_orpd_tpc < mp.math_model
% wgv.math_model_orpd_tpc - Optimal reactive power dispatch (ORPD) **math model** 
%                           for TPC formulations.
%
% Provide implementations for adding system variables to the mathematical
% model and creating an interior starting point.

%   MATPOWER
%   Copyright (c) 2021-2025, Power Systems Engineering Research Center (PSERC)
%   by Wilson Gonzalez Vanegas, Universidad Nacional de Colombia Sede Manizales
%
%   This file is part of MATPOWER.
%   Covered by the 3-clause BSD License (see LICENSE file for details).
%   See https://matpower.org for more info.

%     properties
%     end     %% properties

    methods
        function obj = math_model_orpd_tpc()
            obj@mp.math_model();
            obj.element_classes = { @wgv.mme_bus_orpd_tpc, @wgv.mme_gen_orpd_tpc, ...
                @wgv.mme_load_pf_tpc, @wgv.mme_branch_orpd_tpc, @wgv.mme_shunt_orpd_tpc};
        end

        function tag = task_tag(obj)
            %

            tag = 'orpd';
        end

        function name = task_name(obj)
            %

            name = 'Optimal Reactive Power Dispatch';
        end

        function tag = form_tag(obj)
            %

            % if obj.userdata.tpc.quad
            %     form = 'quad';
            % else
            %     form = 'lin';
            % end
            % 
            % tag = ['tpc-' form];

            tag = 'tpc-quad';
        end

        function name = form_name(obj)
            %

            % if obj.userdata.tpc.quad
            %     form = 'QUAD';
            % else
            %     form = 'LIN';
            % end
            % 
            % name = ['TPC-' form];

            name = 'TPC-QUAD';
        end

        function ad = build_aux_data(obj, nm, dm, mpopt)
            %

            %% create aux_data struct
            ad = obj.build_base_aux_data(nm, dm, mpopt);

            %% Calculate Sbus (1-phase buses)
            if nm.elements.has_name('bus')
                nb1p = nm.node.idx.N.bus;
            end           

            if nm.elements.has_name('load')                
                if nm.userdata.ishybrid
                    Cload1p = nm.elements.load.C(1:nb1p,:);
                else
                    Cload1p = nm.elements.load.C;
                end
                Sload1p = nm.elements.load.s;
            else
                Cload1p = [];
                Sload1p = [];
            end

            if isempty(Cload1p)
                if nm.elements.has_name('bus')
                    Sbus1p = zeros(nb1p,1);
                else
                    Sbus1p = [];
                end
            else
                Sbus1p = - Cload1p*Sload1p;
            end
            

            %% Calculate Sbus (3-phase buses)
            if nm.elements.has_name('load3p')
                Cload3p = nm.elements.load3p.C;
                Sload3p = nm.elements.load3p.s;
            else
                Cload3p = [];
                Sload3p = [];
            end            
            
            if isempty(Cload3p)
                if nm.elements.has_name('bus3p')
                    Sbus3p = zeros(nm.node.idx.N.bus3p,1);
                else
                    Sbus3p = [];
                end
            else
                Sbus3p = - Cload3p*Sload3p;
            end

            %% Stack single-phase and three-phase load
            if nm.userdata.ishybrid
                ad.Sload = [Sbus1p; Sbus3p(nb1p+1:end)];
            else
                ad.Sload = -1 * [Sbus1p; Sbus3p];
            end
        end

        function obj = add_system_vars(obj, nm, dm, mpopt)
            %

            %% add network voltage and non-voltage state variables
            vars = horzcat(nm.model_vvars(), nm.model_zvars());
            for vtype = vars
                st = nm.(vtype{1});     %% set type
                d = st.data;
                mmx_i1 = obj.var.N + 1;
                for k = 1:st.NS
                    name = st.order(k).name;
                    idx = st.order(k).idx;
                    if isempty(idx)
                        obj.add_var(name, st.idx.N.(name), d.v0.(name), d.vl.(name), d.vu.(name), d.vt.(name));
                    else
                        if all(cell2mat(idx) == 1)
                            dim = size(st.idx.N.(name));
                            if dim(end) == 1, dim(end) = []; end   %% delete trailing 1
                            obj.init_indexed_name('var', name, num2cell(dim));
                        end
                        sn = struct('type', {'()'}, 'subs', {idx});
                        sc = sn;
                        sc.type = '{}';
                        N = subsref(st.idx.N.(name), sn);
                        v0 = subsref(d.v0.(name), sc);
                        vl = subsref(d.vl.(name), sc);
                        vu = subsref(d.vu.(name), sc);
                        vt = subsref(d.vt.(name), sc);
                        obj.add_var(name, idx, N, v0, vl, vu, vt);
                    end
                end
                mmx_iN = obj.var.N;
                obj.aux_data.var_map{end+1} = ...
                    {vtype{1}, [], [], [], mmx_i1, mmx_iN, []};
            end
        end

        function add_node_balance_constraints(obj, nm, dm, mpopt)
            %
            
            A = sparse(0);               %% initialize constraint matrix
            b = obj.aux_data.Sload;   %% vector of constraints
            
            % Build balance equations for all elements except TD Interfaces
            % and generators (both 1p and 3p)
            for e = 1:length(nm.elements)
                nme = nm.elements{e};
                if ~isempty(nme.D) && ~strcmp(nme.name,'buslink') && ...
                        ~strcmp(nme.name,'gen') && ~strcmp(nme.name,'gen3p')
                    Ae = [nme.C 1j*nme.C] * [nme.D nme.D]';
                    A = A + Ae;
                end                
            end

            % Include port injections of single-phase generators
            if nm.elements.has_name('gen')
                nme = nm.elements.gen;
                A = A + -1*[nme.C 1j*nme.C] * [nme.D nme.D]';
            end

            % Include port injections of three-phase generators
            if nm.elements.has_name('gen3p')
                nme = nm.elements.gen3p;
                A = A + -1*[nme.C 1j*nme.C] * [nme.D nme.D]';
            end
            
            % Include port injections of TD Interfaces
            if nm.elements.has_name('buslink')
                nme = nm.elements.buslink;
                A = A + nme.C*nme.N*[nme.D nme.D]';
            end
            
            % Add power balance equations in terms of non-voltage state
            % variables (linear constraints). Each port injection is
            % defined independently by each element class in a subsequent
            % stage
            
            vs_zr = nm.zr.order;
            vs_zi = nm.zi.order;
            obj.lin.add(obj.var,'Pmis',real(A),real(b),real(b),vs_zr);
            obj.lin.add(obj.var,'Qmis',imag(A),imag(b),imag(b),vs_zi);
        end

        function x0 = interior_x0(obj, mm, nm, dm, x0)
            %

            if nargin < 5 || isempty(x0)
                %% generic interior point
                [x0, xmin, xmax] = mm.params_var();     %% init var & bounds
                s = 1;                      %% set init point inside bounds by s
                lb = xmin; ub = xmax;
                lb(xmin == -Inf) = -1e10;   %% replace Inf with numerical proxies
                ub(xmax ==  Inf) =  1e10;   %% temporarily to avoid errors in next line
                x0 = (lb + ub) / 2;         %% set x0 mid-way between bounds
                k = find(xmin == -Inf & xmax < Inf);    %% if only bounded above
                x0(k) = xmax(k) - s;                    %% set just below upper bound
                k = find(xmin > -Inf & xmax == Inf);    %% if only bounded below
                x0(k) = xmin(k) + s;                    %% set just above lower bound
            end

            %% allow each node or state creating element to initialize its vars
            for k = 1:length(nm.elements)
                nme = nm.elements{k};   %% network model element
                if nme.nn || nme.nz     %% element creates nodes or states
                    mme = nme.math_model_element(mm);   %% math model element
                    if ~isempty(mme)
                        x0 = mme.interior_x0(mm, nm, dm, x0);
                    end
                end
            end
        end

        function varef1 = interior_va(obj, nm, dm)
            %

            %% return scalar va equal to angle of first reference node
            ad = obj.aux_data;
            ref1 = ad.ref(1);
            varef1 = ad.va(ref1);
        end

        

        % -----------------------------------------------------------------
        % ************ OVERRIDDEN METHODS FROM PARENT CLASSES ************
        % -----------------------------------------------------------------

        function nm = network_model_x_soln(obj, nm)
            % THIS METHOD OVERRIDES THE CORRESPONDING PARENT METHOD. 
            % Convert solved state from math model to network model solution.
            % ::
            %
            %   nm = mm.network_model_x_soln(nm)
            %
            % Input:
            %   nm (mp.net_model) : network model object
            %
            % Output:
            %   nm (mp.net_model) : updated network model object
            %
            % Calls convert_x_m2n() which is defined in a subclass of
            % mp.mm_shared_pfcpf allowing it to be shared across math 
            % models for different tasks (PF and CPF).
            
            [nm.soln.u, nm.soln.z, nm.soln.x] = ...    % sonln.u (difference with parent)
                obj.convert_x_m2n(obj.soln.x, nm);     
        end

        function [u, z, x] = convert_x_m2n(obj, mmx, nm)
            % convert_x_m2n - Convert math model state to network model state.
            % ::
            %
            %   x = mm.pf_convert(mmx, nm)
            %   [v, z] = mm.pf_convert(mmx, nm)
            %   [v, z, x] = mm.pf_convert(mmx, nm)
            %   ... = mm.pf_convert(mmx, nm, only_v)

            %% update v_, z_ from mmx
            nm_vars = obj.update_nm_vars(mmx, nm);
            u = [nm_vars.va; nm_vars.lnvm];
            z  = [nm_vars.zr; nm_vars.zi];            

            %% prepare return values
            if nargout < 2
                u = [u; z];
            elseif nargout > 2
                x = [u; z];
            end
        end

        function opt = solve_opts(obj, nm, dm, mpopt)
            %

            opt = struct('alg', mpopt.orpd.tpc.solver, ...
                         'verbose', mpopt.verbose, ...
                         'knitro_opt', knitro_options);
        end
    end     %% methods
end         %% classdef

classdef mme_xfmr3p_orpd_tpc < wgv.mme_xfmr3p_pf_tpc
% wgv.mme_xfmr3p_orpd_tpc - Math model element for 3-phase transformer for 
%                           TPC-based Optimal Reactive Power Dispacht (ORPD).
%
% Math model element class for 3-phase transformer elements for ORPD task
...
%     end     %% properties
    ...
    methods
        function obj = add_constraints(obj, mm, nm, dm, mpopt)
            ...
            %% 1) Port nijection constraints

            % variables sets
            vs_zr = struct('name',{'Va3', 'Va3', 'Va3', ...
                                   'LnVm3', 'LnVm3', 'LnVm3', ...
                                   'Pxfmr3p', 'Pxfmr3p', 'Pxfmr3p', 'Pxfmr3p', 'Pxfmr3p', 'Pxfmr3p', ...
                                   }, ...
                            'idx',{{1}, {2}, {3}, {1}, {2}, {3}, ...
                                   {1} {2}, {3}, {4}, {5}, {6}} ...
                                   );
            vs_zi = struct('name',{'Va3', 'Va3', 'Va3', ...
                                   'LnVm3', 'LnVm3', 'LnVm3', ...                                   
                                   'Qxfmr3p', 'Qxfmr3p', 'Qxfmr3p', 'Qxfmr3p', 'Qxfmr3p', 'Qxfmr3p' ...
                                   }, ...
                            'idx',{{1}, {2}, {3}, {1}, {2}, {3}, ...
                                   {1} {2}, {3}, {4}, {5}, {6}} ...
                                   );
            ...
            [Q_P, Q_Q, B_P, B_Q] = obj.port_inj_xfmr3p(nme, 3*nm.elements.bus3p.nk);
            l = nme.s;

            % add quadratic constraints for branch active and reactive power port injections
            mm.qcn.add(mm.var, 'Pxfmr3p_inj', Q_P, B_P, -real(l), -real(l), vs_zr);
            mm.qcn.add(mm.var, 'Qxfmr3p_inj', Q_Q, B_Q, -imag(l), -imag(l), vs_zi);

            %% 2) Capacity constraints

            % define quadratic forms for three-phase line capacity
            Q_Smax = obj.capacity_constraints_xfmr3p(dme, nme);
            B_max = [];
            ...
            if ~isempty(Q_Smax)
                for p = 1:nme.np
                    vs = struct('name',{'Pxfmr3p', 'Qxfmr3p'}, ...
                                        'idx',{{p}, {p}});

                    mm.qcn.add(mm.var,sprintf('Smax_xfmr3p_%d',p),Q_Smax,B_max,-l_max,l_max,vs);                    
                end
            end
        end

        function [Qr, Qi, Br, Bi] = port_inj_xfmr3p(obj, nme, nb3p)
            %
        ...
        function Q_Smax = capacity_constraints_xfmr3p(obj,dme, nme)
            %
